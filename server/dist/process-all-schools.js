    ...
    try {
        ...
        for (let i = 0; i < rows.length; i++) {
            ...
            try {
                ...
                if (schoolData.town)
                    towns.add(schoolData.town);
                if (schoolData.location)
                    locations.add(schoolData.location);
                if (schoolData.ward)
                    wards.add(schoolData.ward);
        ...
        filterData.states.forEach(state => {
            const count = stats.schoolsByState[state] || 0;
        ...
        Object.entries(stats.schoolsByLGA)
            ...
            .slice(0, 10)
            .forEach(([lga, count]) => {
            console.log(`  ${lga}: ${count.toLocaleString()} schools`);
        });
        console.log('\n📚 Categories Found:');
        filterData.categories.forEach(category => {
            const count = stats.schoolsByCategory[category] || 0;
...
exports.default = processAllSchoolsFromExcel;
